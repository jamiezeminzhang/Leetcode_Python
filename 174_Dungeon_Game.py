# -*- coding: utf-8 -*-
"""
Created on Sun Feb 14 15:26:02 2016

174. Dungeon Game My Submissions Question
Total Accepted: 21270 Total Submissions: 105393 Difficulty: Hard
    ...
    def calculateMinimumHP(self, dungeon):
        ...
        for i in range(m-1,-1,-1):
            for j in range(n-1,-1,-1):
                if i == m-1 and j == n-1:
                    matrix[i][j] = max(1,1-dungeon[i][j])
                elif i == m-1 and j <= n-2:
                    matrix[i][j] = max(1,1-dungeon[i][j],matrix[i][j+1]-dungeon[i][j] )
                elif i <= m-2 and j == n-1:
                    matrix[i][j] = max(1,1-dungeon[i][j],matrix[i+1][j]-dungeon[i][j])
                else:
                    matrix[i][j] = max(1,1-dungeon[i][j], min(matrix[i][j+1],matrix[i+1][j])-dungeon[i][j]  )
