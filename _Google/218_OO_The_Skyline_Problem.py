...
*** 
    ...
    event中第二部分加上的意义在于写出所有的楼的右边的轮廓。考虑只有一个楼的情况就清楚了。
    对于每个新来的 x ，如果当前堆顶元素的楼高和res中最后一个楼高不同，意味着堆最高楼高更新了，新的pair加入res

*** 更新：
1. 按code中方法排序， 把右侧左边单列出来排到后面
2. 循环每个楼， 每次来个新楼，比较一下新楼的l和堆顶元素的r。如果l>r，堆顶元素出栈。
3. 如果不是单列出来的右边左边的tuple，那么新楼是真实的楼。将新楼的楼高和r入堆。
4. 如果res中最后一个数据不是当前堆顶元素，那么加入当前堆顶元素的值。
